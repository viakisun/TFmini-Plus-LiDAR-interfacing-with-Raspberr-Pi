...
class ManualPage(SettingPage):
    ...
    def init_UI(self):
        ...
        btn_spray_on = tk.Button(self.frame, image=self.img_btn_spray_on, relief=RELIEF_STYLE, bd=0, highlightthickness=0, bg=self.COLOR_BUTTON_BACKGROUND, command=lambda: self.spray_on())
        btn_spray_off = tk.Button(self.frame, image=self.img_btn_spray_off, relief=RELIEF_STYLE, bd=0, highlightthickness=0, bg=self.COLOR_BUTTON_BACKGROUND, command=lambda: self.spray_off())
